...
USE [BEFS_ADMIN]
GO
/****** Object:  StoredProcedure [dbo].[BEFSUP_LIMS1]    Script Date: 04-02-2026 10:53:07 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- Converted SQL Server 2019 Stored Procedure from Oracle's BEFSUP_LIMS
ALter PROCEDURE [dbo].[BEFSUP_LIMS1_DEMO] 
    @pbnow DATETIME,
    @pbdate DATETIME,
    @pedate DATETIME
AS
BEGIN
    ...
    DECLARE @loopit INT = 0,
            @NEXTKEYSC INT,
            @V_INENVACC VARCHAR(2),
            @V_INLABNUM INT,
            @V_INSTA VARCHAR(18),
            @V_INDATE VARCHAR(8),
            @V_INTIM VARCHAR(4),
            @V_INDEPTH VARCHAR(4),
            @V_INDEPTHxx VARCHAR(4),
            @V_INOCHEMNM VARCHAR(10),
            @V_INORAWRESULT VARCHAR(10),
            @V_LCHEMNM VARCHAR(10),
            @V_INRESULTVAL FLOAT,
            @V_INRESULTCD VARCHAR(1),
            @V_UNITS VARCHAR(6),
            @V_INDATE2 VARCHAR(8),
            @V_LASMPLID INT,
            @DATEOFX VARCHAR(20),
            ...
            @XFIND INT,
            @XDATE_ERR VARCHAR(3),
            @NEWONES NVARCHAR(MAX) = '',
            @OLDONES NVARCHAR(MAX) = '',
            @BADSTAS NVARCHAR(MAX) = '',
            @TWARNINGS INT = 0,
            @TOLD INT = 0,
            @TNEW INT = 0,
            @TOUT INT = 0,
    ...
    SELECT TOP 1 @NEXTKEYSC = LASMPLID FROM dbo.SC_CHM_ALL ORDER BY LASMPLID DESC;
    IF @NEXTKEYSC IS NULL RETURN;

    SET @DATEOFX = FORMAT(@pbnow, 'yyyy-MM-dd HH:mm:ss');
    SET @DSTAMPX = 'CAST(''' + @DATEOFX + ''' AS DATETIME)';


    DECLARE cur_SCINALL CURSOR FOR
        SELECT SITE_ID, LAB_NUMBER, DATE_COLLECTED, TIME_COLLECTED, CMP, ORANAME, RAW_RESULT,FINAL_RESULT, RSLT_UNITS_UPPER, FLD_DEPTH, AUNITS, ACCT_NAME
        FROM dbo.SCINFINUS
        WHERE LASTUP_DAT_DT > @pbdate AND LASTUP_DAT_DT <= @pedate AND FORMAT(DATE_COLLECTED,'yyyyMM') >= '201312'
        ORDER BY SITE_ID, DATE_COLLECTED, TIME_COLLECTED, CMP;

    DECLARE @site_id VARCHAR(18), @lab_number INT, @date_collected DATE, @time_collected VARCHAR(5),
            @cmp VARCHAR(10), @oraname VARCHAR(10),@raw_result VARCHAR(10), @final_result VARCHAR(10), @rslt_units VARCHAR(6),
            @fld_depth FLOAT, @aunits VARCHAR(6), @acct_name VARCHAR(50);

    OPEN cur_SCINALL;
    FETCH NEXT FROM cur_SCINALL INTO @site_id, @lab_number, @date_collected, @time_collected, @cmp, @oraname,@raw_result, @final_result, @rslt_units, @fld_depth, @aunits, @acct_name;

    WHILE @@FETCH_STATUS = 0 AND @TCNT <= 99998
    BEGIN
        SET @TCNT += 1;
        SET @final_result = REPLACE(@final_result, ',', '');

        SET @V_INENVACC = LEFT(@site_id, 2);
        SET @V_INLABNUM = @lab_number;
        SET @V_INSTA = ISNULL(NULLIF(@site_id, ''), 'SCNULL');
        IF RIGHT(@V_INSTA, 2) = '_D' SET @V_INSTA = LEFT(@V_INSTA, LEN(@V_INSTA) - 2);
        IF @V_INSTA = 'SC106_D' SET @V_INSTA = 'SC106';

        SET @V_INDATE = FORMAT(@date_collected, 'yyyyMMdd');
        SET @V_INDATE2 = @V_INDATE;
        SET @V_INTIM = LEFT(@time_collected, 2) + SUBSTRING(@time_collected, 4, 2);
        SET @V_LCHEMNM = LEFT(@cmp, 10);
        SET @V_INOCHEMNM = @oraname;
        SET @V_INORAWRESULT = @raw_result
        SET @V_INRESULTVAL = TRY_CAST(@final_result AS FLOAT);
        SET @V_INRESULTCD = CASE WHEN LEFT(@final_result, 1) IN ('<', '>') THEN LEFT(@final_result, 1) ELSE ' ' END;
        IF @V_INRESULTCD <> ' ' SET @V_INRESULTVAL = TRY_CAST(SUBSTRING(@final_result, 2, LEN(@final_result)) AS FLOAT);
        SET @V_UNITS = LEFT(@rslt_units, 6);

        IF @V_INENVACC = 'SC' SET @V_INDEPTH = '00.1';
        IF @acct_name = 'Lake Chemistry'
        BEGIN
            SET @V_INDEPTHxx = FORMAT(@fld_depth, '0.0');
            WHILE LEN(@V_INDEPTHxx) < 4 SET @V_INDEPTHxx = '0' + @V_INDEPTHxx;
            SET @V_INDEPTH = @V_INDEPTHxx;
            WHILE LEN(@V_INSTA) < 6 SET @V_INSTA = '0' + @V_INSTA;
            IF LEFT(@V_INSTA, 2) <> 'LM' SET @V_INSTA = 'LM' + @V_INSTA;
        END
        IF @V_INDEPTH IS NULL SET @V_INDEPTH = '0000';

        IF UPPER(@V_UNITS) = 'UG/L' AND UPPER(@aunits) = 'MG/L' SET @V_INRESULTVAL = @V_INRESULTVAL / 1000.0;

        IF @V_LCHEMNM IS NULL
        BEGIN
            INSERT INTO dbo.SCUPRPT VALUES (@DATEOFX, @INCVAL, 'D:' + @V_INDATE + ' T:' + @V_INTIM + ' d:' + @V_INDEPTH + ' No Analytes (Dry?) - ' + ISNULL(@cmp,''), 'W', @V_INSTA);
            SET @INCVAL += 1; SET @TWARNINGS += 1;
            FETCH NEXT FROM cur_SCINALL INTO @site_id, @lab_number, @date_collected, @time_collected, @cmp, @oraname,@raw_result, @final_result, @rslt_units, @fld_depth, @aunits, @acct_name;
            CONTINUE;
        END

        IF @V_INOCHEMNM IS NULL
        BEGIN
            INSERT INTO dbo.SCUPRPT VALUES (@DATEOFX, @INCVAL, 'D:' + @V_INDATE + ' T:' + @V_INTIM + ' d:' + @V_INDEPTH + ' No Oracle Field FOR: ' + ISNULL(@cmp,''), 'W', @V_INSTA);
            SET @INCVAL += 1; SET @TWARNINGS += 1;
            FETCH NEXT FROM cur_SCINALL INTO @site_id, @lab_number, @date_collected, @time_collected, @cmp, @oraname, @raw_result,@final_result, @rslt_units, @fld_depth, @aunits, @acct_name;
            CONTINUE;
        END

        -- OUTLIER DETECTION CALL
        EXEC dbo.ANISTATS1 'ANI', @V_INSTA, @V_INOCHEMNM, 0.95, @pperval OUTPUT, @pmaxval OUTPUT, @pstd OUTPUT;

        IF @V_INOCHEMNM = 'DISOXY' AND @V_INRESULTVAL < 5
        BEGIN
            INSERT INTO dbo.SCUPRPT VALUES (@DATEOFX, @INCVAL, 'D:' + @V_INDATE + ' T:' + @V_INTIM + ' d:' + @V_INDEPTH + ' DO<5: ' + @V_INOCHEMNM + ' Sample Value: ' + CAST(@V_INRESULTVAL AS NVARCHAR), 'O', @V_INSTA);
            SET @INCVAL += 1; SET @TOUT += 1;
        END

        IF @V_INRESULTVAL > @pmaxval AND ISNULL(@V_INRESULTCD, '') <> '>'
        BEGIN
            INSERT INTO dbo.SCUPRPT VALUES (@DATEOFX, @INCVAL, 'D:' + @V_INDATE + ' T:' + @V_INTIM + ' d:' + @V_INDEPTH + ' OLD MAX: ' + @V_INOCHEMNM + ' ' + CAST(ROUND(@pmaxval, 3) AS VARCHAR(50)) + ' Sample Value: ' + CAST(@V_INRESULTVAL AS NVARCHAR), 'O', @V_INSTA);
            SET @INCVAL += 1; SET @TOUT += 1;
        END
        ELSE IF @V_INRESULTVAL > @pperval AND ISNULL(@V_INRESULTCD, '') <> '>'
        BEGIN
            INSERT INTO dbo.SCUPRPT VALUES (@DATEOFX, @INCVAL, 'D:' + @V_INDATE + ' T:' + @V_INTIM + ' d:' + @V_INDEPTH + ' 95th%: ' + @V_INOCHEMNM + ' ' + TRIM(CAST(ROUND(@pperval, 3) AS VARCHAR(50))) + ' Sample Value: ' + CAST(@V_INRESULTVAL AS NVARCHAR), 'O', @V_INSTA);
            SET @INCVAL += 1; SET @TOUT += 1;
        END
        ELSE IF @V_INRESULTVAL > @pstd AND ISNULL(@V_INRESULTCD, '') <> '>'
        BEGIN
            INSERT INTO dbo.SCUPRPT VALUES (@DATEOFX, @INCVAL, 'D:' + @V_INDATE + ' T:' + @V_INTIM + ' d:' + @V_INDEPTH + ' AVG+2STD: ' + @V_INOCHEMNM + ' ' + CAST(ROUND(@pstd, 3) AS VARCHAR(50)) + ' Sample Value: ' + CAST(@V_INRESULTVAL AS NVARCHAR), 'O', @V_INSTA);
            SET @INCVAL += 1; SET @TOUT += 1;
        END

        -- INSERT or UPDATE to SC_CHM_ALL
        IF EXISTS (SELECT 1 FROM dbo.SC_CHM_ALL WHERE SITE_NAME = @V_INSTA AND COL_DATEX = @V_INDATE AND COL_TIME = @V_INTIM AND DEPTH = @V_INDEPTH)
        BEGIN
            SET @sql = 'UPDATE dbo.SC_CHM_ALL SET ' + QUOTENAME(@V_INOCHEMNM) + '=' + CAST(@V_INRESULTVAL AS NVARCHAR) + ',' + QUOTENAME(@V_INOCHEMNM + 'R') + '=''' + ISNULL(@V_INRESULTCD,'') + ''', DSTAMP=' + @DSTAMPX + ' WHERE SITE_NAME=''' + @V_INSTA + ''' AND COL_DATEX=''' + @V_INDATE + ''' AND COL_TIME=''' + @V_INTIM + ''' AND DEPTH=''' + @V_INDEPTH + '''';
            EXEC sp_executesql @sql;
            SET @TOLD += 1;
        END
        ELSE
        BEGIN
            SET @sql = 'INSERT INTO dbo.SC_CHM_ALL (PROG_CODE, DSTAMP, LABNUM, SITE_NAME, COL_DATEX, COL_DATE, COL_TIME, DEPTH, LASMPLID, ' + QUOTENAME(@V_INOCHEMNM) + ', ' + QUOTENAME(@V_INOCHEMNM + 'R') + ') VALUES (''' + @V_INENVACC + ''',' + @DSTAMPX + ',' + CAST(@V_INLABNUM AS NVARCHAR) + ',''' + @V_INSTA + ''',''' + @V_INDATE + ''',''' + FORMAT(CAST(@V_INDATE AS DATE), 'yyyy-MM-dd') + ''',''' + @V_INTIM + ''',''' + @V_INDEPTH + ''',' + CAST(@NEXTKEYSC AS NVARCHAR) + ',' + CAST(@V_INRESULTVAL AS NVARCHAR) + ',''' + ISNULL(@V_INRESULTCD, '') + ''')';
            EXEC sp_executesql @sql;
            SET @TNEW += 1;
            SET @NEXTKEYSC += 1;
        END
    ...
    CLOSE cur_SCINALL;
    DEALLOCATE cur_SCINALL;

    -- Final log entry
    INSERT INTO dbo.SCUPRPT VALUES (
        @DATEOFX, 0, 'Added: ' + CAST(@TNEW AS NVARCHAR) + ' Updated: ' + CAST(@TOLD AS NVARCHAR) + ' Warnings: ' + CAST(@TWARNINGS AS NVARCHAR) + ' Outliers: ' + CAST(@TOUT AS NVARCHAR) +
        ' FROM: ' + CONVERT(NVARCHAR, @pbdate, 120) + ' TO: ' + CONVERT(NVARCHAR, @pedate, 120),
        'A', ''
    );

    SET @PDATE = CAST(@DATEOFX AS DATETIME);
    --EXEC dbo.Calcions @PDATE, 'N';
END;
